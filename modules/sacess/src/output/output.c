...
void initprintfile_(void *exp1_, double *best, int *par, int *idp, double *currenttime, long *evals, int *master) {
    ...
    experiment_total *exp1 = (experiment_total *) exp1_;
    output_struct *output = exp1->output;

    // OLD: cadea2 = (char *) malloc(400* sizeof (char));

    (*output).st3 = 0.0;

    output->point1 = *currenttime;

    if (exp1[0].test.output != 1) return;

    char *sacess_output_path = (char *) calloc(strlen(exp1[0].test.output_graph) + strlen("/sacess") + 1, sizeof(char));
    strcpy(sacess_output_path, exp1[0].test.output_graph);
    strcat(sacess_output_path, "/sacess");
    mkdir(sacess_output_path, 0777);
    free(sacess_output_path);
    ...
    if (*par == 0) {
        ...
        sprintf(exp1[0].test.output_graph, "%s/sacess/convergence_id%d.csv", temp,*idp);

	    free(temp);
    } else if (exp1[0].execution.initpath == 1) {
        int initp=1;
        ...
        sprintf(exp1[0].test.result_output, "%s/sacess/result", temp);
        sprintf(exp1[0].test.log_output, "%s/sacess/logfile_id%d", temp,*idp);
        if (*idp == 0)  sprintf(exp1[0].test.log_percentage, "%s/sacess/percentage_id%d.csv", temp,*idp);
    ...
    FILE *p3 = fopen((const char *) exp1[0].test.output_graph, "a");
