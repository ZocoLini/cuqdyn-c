%% generateSyntheticData_AP.m
% Generate a synthetic alpha-pinene CSV in EXAMPLES/AP/data.
%
...
time_points = [0, 1230, 3060, 4920, 7800, 10680, 15030, 22620, 36420]';
true_parameters = [5.93e-05, 2.96e-05, 2.05e-05, 2.75e-04, 4.00e-05];
initial_conditions = [100, 0, 0, 0, 0];
is_observed = [1, 1, 1, 1, 0];
noise_level_percent = 10.0;
rng_seed = 101;
min_observed_value = 0;

output_filename = 'AP_synthetic_data_10_partobs1_4.csv';
make_plots = true;
...
if make_plots
    plot_synthetic_data(T, Y_true, Y_data, is_observed, 'AP');
end

function Y_data = apply_synthetic_noise(Y_true, is_observed, noise_level_percent, ...
        min_observed_value)
    Y_data = Y_true;
    sigma = cuqdyn_synthetic_sigma_from_trajectory(Y_true, find(is_observed), ...
        noise_level_percent);
