...
# MPI stage of the validation suite: the MPI implementation of the algorithm
# is validated through its defining invariant. sacess reseeds its RNG on
# every solver invocation, so with a fixed SACESS_SEED each LOO refit is
# deterministic no matter which rank runs it - the result must not depend on
# the process count at all.
#
#   validation/mpi/test-mpi.sh [mpi_build_dir]     (default: build-mpi)
#
# Exercises the partially observed lv2 case (m-1 = 30 held-out points, so 2,
# 3 and 5 divide it and 4 does not). Four checks:
#
#   1. np=1 runs and produces a result            (baseline)
#   2. np=2 and np=5 reproduce np=1 byte for byte (sharding invariance)
...
#
# First real run (WSL Ubuntu, gcc 11.4, OpenMPI 4.1): 4/4, np=2 and np=5
# byte-identical to np=1 - the first green evidence for the TODO line
# "MPI untested with partial observability".
...
if [ ! -x "$CLI" ]; then
  echo "${RED}No MPI build at $BUILD_DIR. Run 'scripts/build.sh mpi' first.${NC}"
  exit 1
fi

CONF=example-files/lv2_partobs_cuqdyn_config.xml
# The serial sacess config on every run: under -DMPI each rank runs its own
# serial eSS over its shard, so cooperative settings play no role here.
ESS=example-files/lv2_partobs_ess_serial_config.xml
DATA=example-files/lv2_partobs_paper_data.txt
