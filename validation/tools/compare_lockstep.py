...
Usage (run_validation.sh layer 5 calls it like this):
    python3 compare_lockstep.py lv2 --c-dir OUT/c --matlab-dir OUT/matlab         --settings OUT/settings.txt --report OUT/report.md

The number of launches is the "m" line of the settings file. The cost tolerance
is layer5_cost in the [name] section of references/tolerances.txt, 1e-4 for a
problem without one.
Exit code 0 always: a report.
"""
...
# at the same theta. Measured over 463 024 identical evaluations of lv2: 5e-9 in
# the median and below 1e-6 for 99.996% of them; on the rest - 19 evaluations,
# where the two integrators take a different step sequence - they differ at the
# integration tolerance, up to 3.4e-5 and never beyond 1e-4. The stiff nfkb
# reaches ~3e-4, its trajectories already differing by 5e-4 between integrators
# (layer2_traj). A wrong data slice on either side moves J by ~1/m of its value,
# orders of magnitude more, so a per-model tolerance in
# references/tolerances.txt (key layer5_cost) separates rounding from an
# orchestration bug without hiding one.
J_TOL_DEFAULT = 1e-4
...
def cost_tolerance(here, model):
    """layer5_cost from the [model] section of references/tolerances.txt."""
    path = os.path.join(here, "..", "references", "tolerances.txt")
    if os.path.isfile(path):
        ...
        with open(path) as f:
            for line in f:
                line = line.strip()
                if line.startswith("["):
                    in_section = line == f"[{model}]"
                    continue
                parts = line.split()
                if in_section and len(parts) == 2 and parts[0] == "layer5_cost":
...
def main():
    ...
    ap.add_argument("--c-dir", required=True, help="C results and traces")
    ap.add_argument("--matlab-dir", required=True, help="MATLAB results and traces")
    ap.add_argument("--settings", required=True, help="settings.txt of the run")
    ap.add_argument("--report", required=True, help="markdown report to write")
    args = ap.parse_args()
    c_dir = args.c_dir
    m_dir = args.matlab_dir
    ...
    with open(args.settings) as f:
        for line in f:
            ...
            if len(parts) == 2 and parts[0] == "m":
                m = int(float(parts[1]))
    if m is None:
        raise SystemExit(f"{args.settings}: no 'm' line")
    ...
    lines += [
        ...
        f"{max(all_j) if all_j else float('nan'):.2e} (tolerance {j_tol:.0e}, layer5_cost in tolerances.txt).",
        "",
