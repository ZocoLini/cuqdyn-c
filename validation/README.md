...
| MPI | **Sharding invariance** | With a fixed `SACESS_SEED`, sacess reseeds its RNG on every solver invocation, so each LOO refit is deterministic no matter which rank runs it: the MPI build's result must not depend on the process count. `validation/mpi/test-mpi.sh` runs the partially observed lv2 case at np=1/2/5 (byte-identical results required), checks that np=4 aborts cleanly (4 does not divide the 30 LOO points) and wraps every run in a timeout so a deadlock fails instead of hanging. | a bug in the MPI sharding/gather of `cuqdyn_algo`, or a hang | none | ✅ np=2/5 byte-identical to np=1 |
