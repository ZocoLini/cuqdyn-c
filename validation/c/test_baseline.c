/*
 * Layers 2 and 4 of the MATLAB comparison.
 *
 ...
 *
 * The two are one binary because they share the config, the data file and the
 * per-model context; layer 3 sits between them and has its own harness.
 *
 * The harness only calls functions the cuqdyn-c library already exports;
 * the library itself is untouched.
 *
 *   ./test_baseline <validation_dir> <model> <cuqdyn_config.xml> <data_file>
 *
 * It reads references/<model>.txt (the problem context, the layer-2 sections
 * and the layer-4 sections, see refio.h) and the tolerances of the model in
 * references/tolerances.txt. A missing layer is skipped; if neither is there
 * the exit code is 77 so ctest reports SKIP. Otherwise the exit code is the
 * number of failing comparisons.
 */
...
static SUNMatrix read_matrix(const char *path, const char *section)
{
    FILE *f = open_section(path, section, 1);

    long rows, cols;
    if (!read_long(f, &rows) || !read_long(f, &cols) || rows < 0 || cols < 0)
    {
        fprintf(stderr, "ERROR: bad header in [%s] of %s\n", section, path);
        exit(2);
    }
    ...
    for (long i = 0; i < rows; ++i)
    {
        for (long j = 0; j < cols; ++j)
        {
            ...
            if (!read_double(f, &v))
            {
                fprintf(stderr, "ERROR: short read in [%s] of %s at (%ld,%ld)\n", section, path, i, j);
                exit(2);
            }
...
static double read_kv_double(const char *path, const char *section, const char *key, double fallback)
{
    char v[256];
    if (!lookup_kv(path, section, key, v, sizeof(v)))
    {
        return fallback;
    }
    double out;
    ...
    {
        fprintf(stderr, "ERROR: [%s] of %s: %s is not a number: %s\n", section, path, key, v);
        exit(2);
    }
    return out;
}

/* Every MATLAB export is checked against the shape the problem implies before
 * it is indexed: a stale or truncated file must fail loudly, not read past the
 * end of a SUNMatrix. */
static void expect_shape(SUNMatrix m, long rows, long cols, const char *what)
{
    if (SM_ROWS_D(m) != rows || SM_COLUMNS_D(m) != cols)
    {
        fprintf(stderr, "ERROR: %s is %ldx%ld, expected %ldx%ld\n", what, SM_ROWS_D(m), SM_COLUMNS_D(m), rows, cols);
        exit(2);
    }
}
...
static void run_layer2(const char *ref, const CuqdynData *data, const CuqdynConf *conf, double tol_traj,
                       double tol_sens)
{
    printf("layer 2: ODE + sensitivities at fixed parameters\n");

    SUNMatrix theta_m = read_matrix(ref, "theta_fixed");
    ...
    SUNMatrix traj_exp = read_matrix(ref, "traj"); /* m x nstates */
    ...
    if (m != NV_LENGTH_S(data->times) || n_states != data->n_states)
    {
        printf("  %-28s export is %ldx%ld but the data file has m=%ld, n_states=%ld  **FAIL**\n", "layer2 shape", m,
               n_states, (long) NV_LENGTH_S(data->times), data->n_states);
        g_checks++;
        g_failures++;
    ...
    /* --- sensitivities --- */
    FILE *f = open_section(ref, "sens", 1);
    long sm, sn, sp;
    if (!read_long(f, &sm) || !read_long(f, &sn) || !read_long(f, &sp) || sm != m || sn != n_states || sp != n_params)
    {
        fprintf(stderr, "ERROR: [sens] header disagrees with [traj]/[theta_fixed]\n");
        exit(2);
    }
    ...
    for (long k = 0; k < n_params; ++k)
    {
        ...
        for (long i = 0; i < m; ++i)
        {
            ...
            {
                double expected;
                if (!read_double(f, &expected))
                {
                    fprintf(stderr, "ERROR: short read in [sens] (param %ld)\n", k);
                    exit(2);
                }
...
static void run_layer4(const char *ref, const CuqdynData *data, const CuqdynConf *conf, double tol_conf,
                       double tol_delta, double tol_covp)
{
    ...
    const double alp_matlab = read_kv_double(ref, "meta", "alp", NAN);
    ...
    SUNMatrix obs_idx_m = read_matrix(ref, "observed_idx"); /* 1-based, from MATLAB */
    ...
    if (section_exists(ref, "sigma") && conf->cost.residual_model == RESIDUAL_MODEL_KNOWN_SIGMA)
    {
        SUNMatrix sigma_m = read_matrix(ref, "sigma");
        Diff d;
        diff_init(&d);
    ...
    SUNMatrix theta_m = read_matrix(ref, "theta_hat");
    ...
    SUNMatrix loo_params = read_matrix(ref, "loo_params"); /* (m-1) x n_params */
    expect_shape(loo_params, m - 1, n_params, "[loo_params]");

    SUNMatrix resid_matlab = read_matrix(ref, "resid_loo"); /* (m-1) x n_obs */
    expect_shape(resid_matlab, m - 1, n_obs, "[resid_loo]");
    ...
    FILE *f = open_section(ref, "media_matrix", 1);
    long n_loo, mm, nn;
    if (!read_long(f, &n_loo) || !read_long(f, &mm) || !read_long(f, &nn) || n_loo != m - 1 || mm != m ||
        nn != n_states)
    {
        fprintf(stderr, "ERROR: [media_matrix] header disagrees with the problem shape\n");
        exit(2);
    }
    ...
    for (long k = 0; k < n_loo; ++k)
    {
        for (long i = 0; i < m; ++i)
        {
            for (long j = 0; j < n_states; ++j)
            {
                ...
                if (!read_double(f, &v))
                {
                    fprintf(stderr, "ERROR: short read in [media_matrix] (block %ld)\n", k);
                    exit(2);
                }
    ...
    SUNMatrix media_tot_row = read_matrix(ref, "media_tot"); /* m x nstates */
    expect_shape(media_tot_row, m, n_states, "[media_tot]");
    ...
    SUNMatrix q_low_exp = read_matrix(ref, "q_low");
    expect_shape(q_low_exp, m, n_states, "[q_low]");
    SUNMatrix q_up_exp = read_matrix(ref, "q_up");
    expect_shape(q_up_exp, m, n_states, "[q_up]");
    SUNMatrix cov_p_exp = read_matrix(ref, "cov_p");
    expect_shape(cov_p_exp, n_params, n_params, "[cov_p]");
    SUNMatrix std_y_exp = read_matrix(ref, "std_y"); /* m x nstates */
    expect_shape(std_y_exp, m, n_states, "[std_y]");
    ...
    else
    {
         ...
         * which is why tolerances.txt can widen this one independently. The bands
...
int main(int argc, char *argv[])
{
    if (argc < 5)
    {
        fprintf(stderr, "Usage: %s <validation_dir> <model> <cuqdyn_config.xml> <data_file>\n", argv[0]);
        return 2;
    }
    ...
    /* One reference file per model holds the problem context and the sections
     * of both stages; the tolerances of every model sit in one hand-maintained
     * file next to it. */
    char ref[1024], tol[1024];
    snprintf(ref, sizeof(ref), "%s/references/%s.txt", root, model);
    snprintf(tol, sizeof(tol), "%s/references/tolerances.txt", root);

    const int have2 = section_exists(ref, "theta_fixed");
    const int have4 = section_exists(ref, "theta_hat");

    if (!have2 && !have4)
    {
        printf("No MATLAB references for %s in %s.\n", model, ref);
        printf("Generate them with MATLAB first: run_validation.sh references --problem=%s\n", model);
    ...
    const double tol_traj = read_kv_double(tol, model, "layer2_traj", 1e-4);
    const double tol_sens = read_kv_double(tol, model, "layer2_sens", 5e-3);
    const double tol_conf = read_kv_double(tol, model, "layer4_conformal", 1e-9);
    const double tol_delta = read_kv_double(tol, model, "layer4_delta", 1e-2);
    const double tol_covp = read_kv_double(tol, model, "layer4_covp", tol_delta);

    printf("model        : %s (under %s)\n", model, root);
    printf("config       : %s\n", config_file);
    printf("data         : %s\n\n", data_file);

    if (have2)
    {
        run_layer2(ref, &data, conf, tol_traj, tol_sens);
    }
    else
    {
    ...
    if (have4)
    {
        run_layer4(ref, &data, conf, tol_conf, tol_delta, tol_covp);
    }
    else
    {
