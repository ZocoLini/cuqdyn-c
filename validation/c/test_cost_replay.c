...
int main(int argc, char *argv[])
{
    ...
    double rel_tol = 1e-3;
    if (argc > 4 && !parse_double(argv[4], &rel_tol))
    {
        fprintf(stderr, "ERROR: rel_tol is not a number: %s\n", argv[4]);
        return 2;
    }
    ...
    if (f == NULL)
    {
        ...
        printf("Generate them with MATLAB first: matlab/gen_cost_replay.m\n");
    ...
    if (!read_long(f, &n_evals) || !read_long(f, &n_params) || n_evals < 0 || n_params != conf->ode_expr.p_count)
    {
    ...
    for (long k = 0; k < n_evals; ++k)
    {
        ...
        {
            if (!read_double(f, &NV_Ith_S(theta, p)))
            {
