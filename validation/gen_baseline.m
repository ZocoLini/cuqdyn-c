...
if ismember(5, layers)
    ...
    for s = seeds(:).'
        ...
        % as on results. run_c_seeds.sh writes the same file on its side.
        tseed = tic;
        res = CUQDyn1_Plus(pb.cost, pb.dynamics, pb.nstates, pb.n_params, ...
            pb.guess_params, pb.lb_params, pb.ub_params, pb.alp, ...
            times, all_state_data, y0, observed_data, observed_idx, ...
...
%                    theta in the layer-5 lock-step check; the stiff nfkb integrates
%                    to 5e-4 (layer2_traj), so its costs agree to 1e-3, the rest to 1e-5
