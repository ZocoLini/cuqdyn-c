...
%
% Writes results/hybrid/hybrid_report_<model>.txt, and the two evaluation logs next to this
...
report_dir = fullfile(here, '..', 'results', 'hybrid');
if ~exist(report_dir, 'dir'), mkdir(report_dir); end
fid = fopen(fullfile(report_dir, sprintf('hybrid_report_%s.txt', model)), 'w');
