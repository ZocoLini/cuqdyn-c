...
%
%   plot_matlab_vs_c('lv2', 'layer6/c/lv2/seed_1/cuqdyn-results.txt')
%       ->  layer6/compare_lv2_matlab_vs_c.png (+ .fig)
%
...
% plotting. The MATLAB bands are the layer-4 export (layer4/matlab/<model>/);
% the C bands come from any cuqdyn-results.txt, typically one layer-5 seed.
...
common = fullfile(here, 'common', 'models', model);
l4 = fullfile(here, 'layer4', 'matlab', model);

% --- MATLAB side (layer-4 export + shared problem context) ---
times = read_m(fullfile(common, 'times.txt'));
obs_idx = read_m(fullfile(common, 'observed_idx.txt'));       % 1-based
ql_m = read_m(fullfile(l4, 'q_low.txt'));
qu_m = read_m(fullfile(l4, 'q_up.txt'));
fit_m = read_m(fullfile(l4, 'media_tot.txt'));
obs_data = read_m(fullfile(l4, 'observed_data.txt'));
...
alp = read_kv(fullfile(common, 'meta.txt'), 'alp');
...
out = fullfile(here, 'layer6', sprintf('compare_%s_matlab_vs_c', model));
