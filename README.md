...
they share the sensitivities and the conformal bands, so the second is one more
quadratic form.
...
write. The parameter covariance is still computed and reported (`CovP_fim`,
`StdY_fim` and their `hybrid` counterparts), so existing `.txt` inputs keep their
bands and gain the FIM diagnostics.
...
>>>>>>> ff14392 (fix: remove plot.py retro-compatibility)
